    ...
    if (data[offset] == 0xFF && data[offset+1] == 0xD8) {
      ...
      while (offset < data.length) {
         if (data[offset] == 0xFF && data[offset+1] == 0xE1 ) {
            if (data.toString('utf8', offset+4, offset+10) != 'Exif\0\0') {
              throw new Error('The Exif data is not valid.');
            }
            var exifData = self.extractExifData(data, offset + 10 );
      ...
      self.imageType = 'TIFF';
      var exifData = self.extractExifData(data, 0 );
      callback(false, exifData);
      return;
      ...
      callback(new Error('The given image is not a JPEG nor TIFF and thus unsupported right now.'));
      return;
    }
  } catch (error) {
    callback(error);
